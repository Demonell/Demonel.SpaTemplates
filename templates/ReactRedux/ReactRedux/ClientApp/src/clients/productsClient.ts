...
export class ProductsClient {
     ...
     */
    get(id: number | null | undefined, name: string | null | undefined, productType: ProductType | null | undefined, deliveryDate_From: Date | null | undefined, deliveryDate_To: Date | null | undefined, materialName: string | null | undefined, skip: number | null | undefined, take: number | null | undefined, sort: string | null | undefined): Promise<TotalListOfProductVm> {
    ...
    protected processGet(response: Response): Promise<TotalListOfProductVm> {
     ...
     * @return Успех! Возвращает объект с информацией о продукте
     */
    get2(id: number): Promise<ProductVm> {
    ...
    protected processGet2(response: Response): Promise<ProductVm> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
    ...
    /**
     * Создание продукта
     * @return Успех! Возвращает созданный объект
     */
    create(command: CreateProductCommand): Promise<ProductVm> {
    ...
    protected processCreate(response: Response): Promise<ProductVm> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 201) {
            return response.text().then((_responseText) => {
            let result201: any = null;
            result201 = _responseText === "" ? null : <ProductVm>JSON.parse(_responseText, this.jsonParseReviver);
            return result201;
            });
        } else if (status === 400) {
            return response.text().then((_responseText) => {
